...
class smokeEffect {
  constructor(viewer: Cesium.Viewer, obj: { lng: number; lat: number }) {
    this.viewer = viewer;
    this.viewModel = {
      emissionRate: 5,
      gravity: 0.0, // 设置重力参数
    ...
    this.particleSystem = '' as unknown as Cesium.ParticleSystem;
    this.entity = this.viewer.entities.add({
      // 选择粒子放置的坐标
      position: Cesium.Cartesian3.fromDegrees(obj.lng, obj.lat, 100),
    });
    this.init();
  }
  ...
  init() {
    ...
    const particleSystem = this.scene.primitives.add(
      new Cesium.ParticleSystem({
        image: smoke, // 生成所需粒子的图片路径
        // 粒子在生命周期开始时的颜色
  ...
  preUpdateEvent() {
    const _this = this;
    this.viewer.scene.preUpdate.addEventListener(function (scene: Cesium.Scene, time: Cesium.JulianDate) {
      // 发射器地理位置
  ...
  applyGravity(p: Cesium.Particle, dt: number) {
...
let smokeEffectInstance: smokeEffect;
...
onMounted(() => {
  ...
  smokeEffectInstance = new smokeEffect(viewer, {
    lng: -117.16,
    lat: 32.71,
  });
...
onUnmounted(() => {
  if (smokeEffectInstance) {
    smokeEffectInstance.remove();
  }
});
</script>
